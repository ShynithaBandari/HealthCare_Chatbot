...
def check_pattern(dis_list, inp):
    pred_list = []
    symptoms = inp.lower().split(',')  # Split multiple symptoms and convert to lowercase
    for symptom in symptoms:
        symptom = symptom.strip()  # Remove extra spaces
        inp_lower = symptom.replace(' ', '_')  # Convert to lowercase and replace spaces with underscores
        patt = f"{inp_lower}"
        regexp = re.compile(patt)
        pred_list.extend([item for item in dis_list if regexp.search(item.lower())])  # Add matched symptoms to pred_list
    pred_list = list(set(pred_list))  # Remove duplicate entries
    if pred_list:
        return 1, pred_list
    else:
        return 0, []
...
def tree_to_code(tree, feature_names):
    ...
    disease_input = input("").lower().split(',')  # Convert input to lowercase and split by commas
    print("Disease input after split:", disease_input)  # Print for debugging

    for symptom in disease_input:
        print("Symptom:", symptom)  # Print for debugging
        conf, cnf_dis = check_pattern(chk_dis, symptom.strip())
        print("Confidence:", conf)  # Print for debugging
        print("Confident diseases:", cnf_dis)  # Print for debugging
        if conf == 1:
    ...
    def recurse(node, depth):
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            if name in disease_input:  # Check if symptom is in the input list
                val = 1
            else:
                val = 0
            if val <= threshold:
